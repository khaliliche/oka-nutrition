"use client";

import FadeIn from "./FadeIn";
import { useOrderModal } from "./OrderModalContext";

type OfferId = "1mois" | "3mois";

type Offer = {
  id: OfferId;
  number: string;
  label: string;
  price: string;
  oldPrice: string;
  image: string;
  description: string;
  popular?: boolean;
};

const offers: Offer[] = [
  {
    id: "1mois",
    number: "01",
    label: "1 mois",
    price: "199 DH",
    oldPrice: "249 DH",
    image: "/images/product-1month.jpg",
    description: "Une boîte pour une cure complète d'un mois.",
  },
  {
    id: "3mois",
    number: "03",
    label: "3 mois",
    price: "349 DH",
    oldPrice: "747 DH",
    image: "/images/product-3months.jpg",
    description: "La cure complète au meilleur tarif.",
    popular: true,
  },
];

export default function Offers() {
  const { selectedOffer, setSelectedOffer, open } = useOrderModal();

  return (
    <section
      id="offers"
      className="bg-navy-light px-4 sm:px-6 lg:px-8 py-16 sm:py-20 lg:py-24"
    >
      <div className="max-w-6xl mx-auto">

        <FadeIn>
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 mb-12 lg:mb-14">

            <div>
              <p className="text-[10px] uppercase tracking-[0.25em] text-brass-light mb-4">
                Votre cure
              </p>

              <h2 className="font-heading text-3xl sm:text-4xl lg:text-[3.2rem] leading-[1] text-white">
                Choisissez votre
                <span className="block italic font-normal">
                  routine.
                </span>
              </h2>
            </div>

            <p className="text-sm text-charcoal-light leading-relaxed max-w-md lg:pb-1">
              Commencez avec un mois ou profitez directement de la cure
              complète de 3 mois à seulement 349 DH.
            </p>

          </div>
        </FadeIn>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6">

          {offers.map((offer, index) => {
            const selected = selectedOffer === offer.id;

            return (
              <FadeIn key={offer.id} delay={index * 0.12}>

                <article
                  onClick={() => setSelectedOffer(offer.id)}
                  className={`group relative cursor-pointer bg-navy rounded-2xl overflow-hidden border transition-all duration-500 ${
                    selected
                      ? "border-brass ring-1 ring-brass"
                      : "border-white/10 hover:-translate-y-1 hover:border-white/25"
                  }`}
                >

                  {offer.popular && (
                    <div className="absolute top-0 right-0 z-10 bg-brass text-white px-4 py-2 text-[8px] uppercase tracking-[0.2em] rounded-bl-lg">
                      Recommandé
                    </div>
                  )}

                  <div className="relative bg-navy-light h-[280px] sm:h-[320px] lg:h-[370px] flex items-center justify-center overflow-hidden">

                    <img
                      src={offer.image}
                      alt={offer.label}
                      className="w-[62%] sm:w-[68%] lg:w-[72%] h-[82%] object-contain transition-transform duration-700 group-hover:scale-[1.04]"
                    />

                    <span className="absolute top-5 left-5 font-heading text-xs text-charcoal-light">
                      {offer.number}
                    </span>

                  </div>

                  <div className="p-6 sm:p-7 lg:p-8">

                    <div className="flex items-start justify-between gap-5">

                      <div>
                        <p className="text-[9px] uppercase tracking-[0.2em] text-charcoal-light mb-2">
                          Cure
                        </p>

                        <h3 className="font-heading text-2xl sm:text-3xl text-white">
                          {offer.label}
                        </h3>
                      </div>

                      <span
                        className={`w-5 h-5 rounded-full border flex items-center justify-center shrink-0 mt-1 ${
                          selected
                            ? "border-brass"
                            : "border-white/25"
                        }`}
                      >
                        {selected && (
                          <span className="w-2.5 h-2.5 bg-brass rounded-full" />
                        )}
                      </span>

                    </div>

                    <div className="flex items-baseline gap-3 mt-5">

                      <span className="font-heading text-3xl sm:text-4xl lg:text-5xl text-white">
                        {offer.price}
                      </span>

                      <span className="text-xs text-charcoal-light line-through">
                        {offer.oldPrice}
                      </span>

                    </div>

                    <p className="text-sm text-charcoal-light leading-relaxed mt-3">
                      {offer.description}
                    </p>

                    <div className="border-t border-white/10 mt-6 pt-5 space-y-2">
                      <p className="text-xs text-white/80">
                        ✓ Livraison gratuite
                      </p>

                      <p className="text-xs text-white/80">
                        ✓ Paiement à la livraison
                      </p>
                    </div>

                    <button
                      type="button"
                      onClick={(e) => {
                        e.stopPropagation();
                        open(offer.id);
                      }}
                      className={`mt-7 w-full py-3.5 rounded-full text-sm font-medium transition-all duration-300 ${
                        selected
                          ? "bg-brass text-white hover:bg-brass-light"
                          : "border border-white/15 text-white hover:border-brass"
                      }`}
                    >
                      Choisir cette cure
                    </button>

                  </div>

                </article>

              </FadeIn>
            );
          })}

        </div>

        <FadeIn delay={0.25}>
          <div className="mt-8 pt-6 border-t border-white/10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">

            <p className="text-xs text-charcoal-light">
              🇲🇦 Livraison gratuite partout au Maroc · Paiement à la réception
            </p>

            <button
              type="button"
              onClick={() => open(selectedOffer)}
              className="group text-sm font-medium text-brass-light"
            >
              <span className="inline-flex items-center gap-2">
                Commander maintenant
                <span className="group-hover:translate-x-1 transition-transform">
                  →
                </span>
              </span>
            </button>

          </div>
        </FadeIn>

      </div>
    </section>
  );
}